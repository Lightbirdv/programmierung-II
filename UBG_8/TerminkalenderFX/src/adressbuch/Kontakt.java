...
public class Kontakt implements Comparable<Kontakt>, Serializable {
    ...
    public Kontakt(String name, String telefon, String email) {
        ...
        if ((name == null || name.isEmpty()) && (telefon == null || telefon.isEmpty())) {
            throw new IllegalArgumentException(
                "Ungültige Eingabe bei der Erstellung von Kontakt: name ist leer"
            );
        }
        if (name == null) {
            name = "";
    ...
    public static Kontakt makeKontakt(String kontaktInfos) {
        try {
            System.out.println(kontaktInfos);
            return new Kontakt(kontaktInfos.split(":")[0],kontaktInfos.split(":")[1],kontaktInfos.split(":")[2]);
