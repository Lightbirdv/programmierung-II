...
public class Adressbuch {
    ...
    /**
    * legt Test Daten an und wir in initialisieren aufgerufen
    */
    ...
    /**
    * speichert alle Kontakte in File zeilenweise
    * @param ausgabeDatei ist das File in die die Kontakte reingeschrieben werden sollen
    * @throws IOException
    */
    public void save(File ausgabeDatei) throws IOException {
        FileWriter writer = new FileWriter(ausgabeDatei);
        BufferedWriter bufferWriter = new BufferedWriter(writer);
        Kontakt[] kontakte = this.getAlleKontakte();
        for(Kontakt k : kontakte){
            bufferWriter.write(k.getKontaktString() + "\n");
        }
        bufferWriter.close();
    }
    
     /**
    * lädt alle Kontakte aus einer Datei
    * @param eingabeDatei ist das File aus dem die Kontakte geladen werden
    * @throws FileNotFoundException
    */
    public void load(File eingabeDatei) throws FileNotFoundException{
        FileReader reader = new FileReader(eingabeDatei);
        BufferedReader readBuffer = new BufferedReader(reader);
        String Line;
        try {
            for(Line = readBuffer.readLine(); Line != null; Line = readBuffer.readLine()) {
                this.addKontakt(Kontakt.makeKontakt(Line));
            }      
